export default class Merge {
    constructor (config, hook) {
        this.hook = hook;
        this.prefixServerUrl = config.get('diff.prefixServerUrl'); // diff 服务地址
        this.prefixDiffFileUrl = config.get('diff.prefixDiffFileUrl'); // diff 文件地址前缀
    }
    ...
    _fetchDiffData (oldUrl, newUrl, callback) {
        ...
        this._xhr(this.prefixDiffFileUrl + newPath.replace(/\//g, '') + oldPath.replace(/\//g, ''), (responseText) => {
            if (responseText && responseText !== '') {
                ...
                // 通知服务器生成增量文件
                this._xhr(this.prefixDiffFileUrl + '?target=' + newPath + '&source=' + oldPath, () => {
                    return true;
                });
            }

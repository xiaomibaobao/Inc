...
var loadAsserts = function (url, type, charset, version, module, callback) {
    ...
    if (store.isExist(module)) {
        var moduleContent = store.getContent(module);
        var moduleVersion = store.getVersion(module);
        var moduleURL = store.getVersion(module);

        if(store.getVersion(module) === version) {
            return eval(moduleContent);
        } else {
            return merge.merge(url, moduleURL, moduleContent, function(content) {
                if(!content) {
...
var loadParallel = function (asserts, callback) {
    ...
    for (var i = 0; i < asserts.length; i++) {
        ...
        if (typeof (current) === 'undefined') {
            console && console.warn && console.warn('In Error :: Module not found: ' + module);
            hook();
            continue;
        }

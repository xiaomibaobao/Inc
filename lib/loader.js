...
var jsCall = function (url, module, callback) {
    setTimeout(function () {
        ...
        if (data && data !== '') {
            try {
                var evalStartTime = new Date().getTime();
                globalEval(modules[url]);
                var evalTime = new Date().getTime() - evalStartTime;
                console.log(url + ' eval 执行时间为：' + evalTime);
                if (window.mta) {
                    window.mta('timing', 'js.incEval', evalTime);
                }
                callback();
            } catch (e) {
...
var loadScript = function (url, module, isStore, version, callback) {
    ...
    // 是否使用本地存储
    if (isStore) {
        var storeStartTime = new Date().getTime();
        var moduleData = store.getItem(module) || {};
        var storeTime = new Date().getTime() - storeStartTime;
        console.log(module + ' 读取 store 执行时间为：' + storeTime);
        if (window.mta) {
            window.mta('timing', 'js.incStore', storeTime);
        }
        ...
        var mergeStartTime = new Date().getTime();
        merge.merge(oldUrl, url, moduleContent, function (content) {
            var mergeTime = new Date().getTime() - mergeStartTime;
            console.log(module + ' merge 执行时间为：' + mergeTime);
            if (window.mta) {
                window.mta('timing', 'js.incMerge', mergeTime);
            }
            if (!content && oldUrl !== '') {
                store.removeItem(module);
